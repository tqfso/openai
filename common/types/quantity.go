...
func (q *Quantity) AsFloor() Quantity {
	data := q.AsApproximateFloat64()
	return *NewQuantity(int64(data), q.Format)
}

func (q *Quantity) AsGiFloor() Quantity {
	align := int64(1024 * 1024 * 1024)
	data := int64(q.AsApproximateFloat64())
	data = data / align * align
	return *NewQuantity(data, BinarySI)
}

func (q *Quantity) AsTiFloor() Quantity {
	align := int64(1024 * 1024 * 1024 * 1024)
	data := int64(q.AsApproximateFloat64())
	data = data / align * align
	return *NewQuantity(data, BinarySI)
}
...
// IsLessEqualZero returns true if the quantity is less or equal zero
func (q *Quantity) IsLessEqualZero() bool {
	if q.d.Dec != nil {
		return q.d.Dec.Cmp(inf.NewDec(0, inf.Scale(0))) <= 0
	}
	return q.i.Cmp(int64Amount{value: 0}) <= 0
}
...
// NewQuantity returns a new Quantity representing the given float value
func NewFloatBinaryQuantity(value float64) *Quantity {
	return &Quantity{
		i:      int64Amount{value: int64(value * 1000), scale: -3},
		Format: BinarySI,
	}
}
